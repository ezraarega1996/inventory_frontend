...
class _BoughtsScreenState extends State<BoughtsScreen> {
  ...
  void _showAddEditDialog({
    ...
    setState(() {
      ...
      _fractionPurchasePriceController.text =
          fractionPurchasePrice?.toString() ?? '';
                                        ...
                                        child: DropdownButton<String>(
                                          ...
                                          onChanged: (value) {
                                            setState(() {
                                              ...
                                              final fraction = item.fractions
                                                  ?.firstWhere(
                                                    (f) => f.id == value,
                                                  );
                                              if (fraction != null) {
                                                _fractionPurchasePriceController
                                                        .text =
                                                    fraction.price.toString();
                                                _fractionSoldPriceController
                                                        .text =
                                                    fraction.price.toString();
                                              }
                                            });
  ...
  Future<void> _saveBought() async {
    if (_formKey.currentState!.validate()) {
      ...
      final boughtProvider = Provider.of<BoughtProvider>(
        context,
        listen: false,
      );
      final itemProvider = Provider.of<ItemProvider>(context, listen: false);
      ...
      if (_editingBoughtId == null) {
        success = await boughtProvider.createBought(
          ...
          fractionPurchasePrice: double.parse(
            _fractionPurchasePriceController.text,
          ),
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: const Text(
              'Are you sure you want to delete this bought item?',
            ),
            actions: [
              TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Bought Items')),
      body: RefreshIndicator(
        onRefresh: _loadBoughts,
        child:
            boughtProvider.isLoading
                      ...
                      child: StatefulBuilder(
                        builder: (context, setState) {
                          ...
                          String selectedFractionId =
                              ...
                              bought.fraction?.id ??
                              '';
                          Fraction? selectedFraction = bought.item?.fractions
                              ?.firstWhere((f) => f.id == selectedFractionId);

                          // Find the bought record for this fraction (if any)
                          Bought? fractionBought = boughtProvider.boughts
                              .firstWhere(
                                (b) =>
                                    b.itemId == bought.itemId &&
                                    b.fractionId == selectedFractionId,
                                orElse: () => bought,
                              );
